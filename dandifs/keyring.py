# NOTICE
#   This file was copied and modified from [dandi-cli] dandi/keyring.py,
...
def keyring_op(
    ...
    try:
        ...
        LOG.info("Default keyring errors on query: %s", e)
        if isinstance(kb, EncryptedKeyring):
            LOG.info(
                "Default keyring is EncryptedKeyring; "
                "abandoning keyring procedure"
            )
            raise
        ...
        if op.exists(kb.file_path):
            LOG.info("EncryptedKeyring file exists; using as keyring backend")
            return (kb, func(kb))
        LOG.info("EncryptedKeyring file does not exist")
        if confirm(
            "Would you like to establish an encrypted keyring?", default=True
        ):
            keyring_cfg = keyringrc_file()
            if keyring_cfg.exists():
                LOG.info("%s exists; refusing to overwrite", keyring_cfg)
            else:
                LOG.info(
                    "Configuring %s to use EncryptedKeyring "
                    "as default backend",
                    keyring_cfg,
                )
...
def keyringrc_file() -> Path:  # noqa: D103
